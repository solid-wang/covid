...
import (
	...
	"github.com/solid-wang/covid/cmd/devops-controller-manager/app/options"
...
func NewControllerManagerCommand() *cobra.Command {
	s := options.NewDevOpsControllerOptions()
...
func NewControllerInitializers() map[string]InitFunc {
	...
	controllers["continuous-integration"] = startContinuousIntegrationController
	controllers["continuous-deployment"] = startContinuousDeploymentController
...
func Run(c *config.CompletedConfig, stopCh <-chan struct{}) error {
	...
	if err := StartWebServer(&c.WebServer); err != nil {
		klog.Fatalf("error starting webserver: %v", err)
	}
