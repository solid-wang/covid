...
type DemoLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Demo, err error)
...
// List lists all Demos in the indexer.
func (s *demoLister) List(selector labels.Selector) (ret []*v1beta1.Demo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Demo))
	})
	return ret, err
}
...
type DemoNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Demo, error)
...
// List lists all Demos in the indexer for a given namespace.
func (s demoNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Demo, err error) {
...
// Get retrieves the Demo from the indexer for a given namespace and name.
func (s demoNamespaceLister) Get(name string) (*v1beta1.Demo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("demo"), name)
	}
	return obj.(*v1beta1.Demo), nil
}
