...
import (
	...
	continuousdeploymentstore "github.com/solid-wang/covid/pkg/registry/devops/continuousdeployment"
	continuousintegrationstore "github.com/solid-wang/covid/pkg/registry/devops/continuousintegration"
	serverversionstore "github.com/solid-wang/covid/pkg/registry/devops/serverversion"
...
func (p StorageProvider) NewRESTStorage(restOptionsGetter generic.RESTOptionsGetter) (genericapiserver.APIGroupInfo, error) {
	apiGroupInfo := genericapiserver.NewDefaultAPIGroupInfo(devops.GroupName, scheme.Scheme, scheme.ParameterCodec, scheme.Codecs)

	if storageMap, err := p.v1Storage(scheme.Scheme, restOptionsGetter); err != nil {
		return genericapiserver.APIGroupInfo{}, err
	} else if len(storageMap) > 0 {
		apiGroupInfo.VersionedResourcesStorageMap[devopsv1.SchemeGroupVersion.Version] = storageMap
	}
...
func (p StorageProvider) v1Storage(scheme *runtime.Scheme, restOptionsGetter generic.RESTOptionsGetter) (map[string]rest.Storage, error) {
	...
	// serverversions
	serverversionStorage, err := serverversionstore.NewREST(scheme, restOptionsGetter)
	if err != nil {
		return storage, err
	}
	storage["serverversions"] = serverversionStorage
	...
	storage["continuousdeployments"] = continuousdeploymentStorage
